...
public class Object implements UniversalBodyModel {
  ...
  public void deserialize(SerializedData data) {
    ...
    if (data.has("counts"))
      this.counts =
          data.applyAsCollection("counts")
              .map(sd -> sd.applyAs(int.class))
              .collect(Collectors.toList());

    if (data.has("subObjects"))
      this.subObjects =
          data.applyAsCollection("subObjects")
  ...
  public String[] getStructure() {
    return new String[] {"uuid", "coins", "sub"};
  }

  @Override
  public String toString() {
    return "Object{" + "uuid=" + uuid + ", coins=" + coins + '}';
  }
