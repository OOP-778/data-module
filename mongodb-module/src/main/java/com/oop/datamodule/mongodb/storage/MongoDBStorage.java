...
public abstract class MongoDBStorage<T extends MongoModelBody> extends Storage<T> {
    ...
    protected void _save(T object) {
        ...
        lock.lockAndUse(() -> {
            ...
            ModelCachedData modelCachedData = getDataCache().computeIfAbsent(key, k -> new ModelCachedData());
            ...
            if (docObject != null) {
                ...
                data.getJsonElement().getAsJsonObject().entrySet().forEach(entry -> {
                ...
                collection.updateOne(filter, new Document("$set", updatedFields), new UpdateOptions().upsert(true));
    ...
    public void remove(T object) {
        onRemove(object);
        StorageInitializer.getInstance().getRunner(true)
                .accept(() -> acquireAndLaterRemove(object, () -> getLock(object).lockAndUse(() -> {
                    MongoCollection<Document> collection = getOrCreateCollection(database, findVariantNameFor(object.getClass()));
                    collection.findOneAndDelete(Filters.eq(object.getIdentifierKey(), object.getKey()));
                })));
    }
