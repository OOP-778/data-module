...
public class DataUtil {
    ...
    public static JsonElement primitiveToString(Object object) {
        ...
        if (clazz == Integer.class)
            return new JsonPrimitive(object.toString() + "i");
    ...
    public static JsonElement wrap(Object object) {
        ...
        if (DataUtil.isPrimitive(object.getClass()))
            ...
            Optional<? extends StorageInitializer.AdapterObject> adapter = StorageInitializer.getInstance().findAdapter(object.getClass());
    ...
    public static <T> T fromElement(JsonElement element, Class<T> clazz) {
        ...
        if (!isPrimitive(clazz)) {
            Optional<? extends StorageInitializer.AdapterObject> adapter = StorageInitializer.getInstance().findAdapter(clazz);
            if (adapter.isPresent())
                return (T) adapter.get().getAdapter().fromJsonTree(element);
            else
                return StorageInitializer.getInstance().getGson().fromJson(element, clazz);
        }
    ...
    private static <T> Object doConversion(Object parsed, Class<T> clazz) {
        ...
        clazz = Primitives.wrap(clazz);
        if ((parsed.getClass() == Double.class || parsed.getClass() == Float.class) && Primitives.wrap(clazz) == Integer.class) {
            if (value.contains(".")) {
