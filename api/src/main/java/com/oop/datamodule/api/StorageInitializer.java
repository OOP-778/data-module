...
public class StorageInitializer {
   ...
   * @return a pair of StorageInitializer instance and hook to run when application shut downs to
   *     clean instance
   */
   ...
   * @param hierarchyEnabled if hierarchy is enabled then it will also work on objects that
   *     implements or extends base class
   * @param adapter adapter which serializes / deserializes objects
   */
  public <T extends Object> void registerAdapter(
      Class<T> clazz, boolean hierarchyEnabled, TypeAdapter<T> adapter) {
  ...
  public <T> Optional<AdapterObject<T>> findAdapter(Class<T> clazz) {
    for (Map.Entry<Class<?>, AdapterObject<?>> entry : adapters.entrySet()) {
      if ((entry.getValue().isHierarchyEnabled() && entry.getKey().isAssignableFrom(clazz))
          || entry.getKey() == clazz) return Optional.of((AdapterObject<T>) entry.getValue());
    }
  ...
  public Gson getGson() {
    if (gson == null) {
      ...
      if (onBuild != null) onBuild.accept(builder);
      gson = builder.create();
    }
