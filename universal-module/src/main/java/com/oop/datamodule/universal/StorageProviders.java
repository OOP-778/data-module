...
public class StorageProviders {
    ...
    static {
        for (Field field : StorageProviders.class.getFields()) {
            ...
            try {
                byName.put(field.getName().toLowerCase().replace("_", ""), (StorageProvider) field.get(null));
    ...
    public static <T extends StorageProvider> T getByName(String name) {
        return (T) Objects.requireNonNull(byName.get(name), "Storage Provider by name: " + name + " not found!");
    }
}
