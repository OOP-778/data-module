...
public class StorageImporter {
  ...
  //        file.exists(), "Failed to import data from " + file.getName() + " cause it doesn't
  // exist!");
  //
  ...
  ////
  ////        Preconditions.checkArgument(
  ////            modelBodyClass != null, "Failed to find model by variant " + key);
  ////
  ////        for (String variantDatum : variantData) {
  ////          JsonObject jsonObject =
  ////              StorageInitializer.getInstance().getGson().fromJson(variantDatum,
  // JsonObject.class);
  ...
  ////
  ////          deserializedData.computeIfAbsent(key, $ -> new ArrayList<>()).add(object);
  ////        }
  ////      }
  ////    } catch (Throwable throwable) {
  ////      throw new IllegalStateException("Failed to import data from " + file.getName(),
  // throwable);
  ////
  ////    } finally {
  ////      inputStream.close();
